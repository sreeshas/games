...
def group_collide(group, other_object):
    groupcopy = set(group)
    hasCollision = False
    for sprite in groupcopy:
        if (other_object.collide(sprite) == True):
            group.remove(sprite)
            hasCollision = True
    return hasCollision
...
def draw(canvas):
    global time, started, rock_group, lives, score, missile_group
...
def rock_spawner():
    global a_rock, rock_group, started
    if started == False:
        return
...
def process_sprite_group(spriteset, canvas):
    spritesetcopy = set(spriteset)
    for sprite in spritesetcopy:
        sprite.draw(canvas)
...
def group_group_collide(rockgroup, missilegroup):
    count = 0
    rockgroupcopy = set(rockgroup)
    for rock in rockgroupcopy:
        if group_collide(missilegroup, rock) == True:
